...
//        c.drawText("正在打开书本...", mHeight / 2, 0, waitPaint);
     ...
     * wym 应该跟随语音显示的文字
     */
    public int[] indexRangeOfVoice = new int[2];

    public void onDrawTXT(Bitmap bitmap, List<String> m_lines, Boolean updateCharter) {
        ...
        if (m_lines.size() > 0) {
            ...
            for (String strLine : m_lines) {
                ...
                if (range > 0 || (range < -1 && range > -indexRangeOfVoice.length)) {        // 这些文字在语音播放时候需要显示背景
                    ...
                    voicePaint.setColor(Color.argb(180, 17, 89, 235));
                    c.drawRect(measureMarginWidth, y, measureMarginWidth + getFontSize(), y + getFontSize(), voicePaint);
                }
...
//                word.append(strLine);
        ...
        c.drawText(strPage, mWidth - nPercentWidth, mHeight - statusMarginBottom, mBatterryPaint);//x y为坐标值
//        c.drawText(date, marginWidth ,mHeight - statusMarginBottom, mBatterryPaint);

        //画书名
        mPaint.setColor(Color.parseColor("#7B6645"));
        c.drawText(CommonUtil.subString(bookName, 12), marginWidth, statusMarginTop + mBatterryFontSize, mPaint);
        //画章
...
//        }
    ...
    public void prePage() {
        ...
        onDrawTXT(mBookPageWidget.getCurPage(), currentPage.getLines(), true);
        currentPage = getPrePage();
        onDrawTXT(mBookPageWidget.getNextPage(), currentPage.getLines(), true);
    }
    ...
    public void nextPage() {
        ...
        onDrawTXT(mBookPageWidget.getNextPage(), currentPage.getLines(), true);
        LogUtils.e("nextPage" + "nextPagenext");
    }
...
//        m_mbBufLen = 0;
    ...
    private class BookTask extends AsyncTask<Long, Void, Boolean> {
        ...
        protected void onPostExecute(Boolean result) {
            ...
            LogUtils.e("onPostExecute" + isCancelled() + "");
            if (isCancelled()) {
                return;
            }
            if (result) {
...
//                m_mbBufLen = mBookUtil.getBookLen();
    ...
    public TRPage getNextPage() {
        ...
        trPage.setIndex(currentPage.getIndex() + 1);
        trPage.setBegin(currentPage.getEnd() + 1);
        LogUtils.e("begin" + currentPage.getEnd() + 1 + "");
        trPage.setLines(getNextLines());
        LogUtils.e("end" + mBookUtil.getPosition() + "");
    ...
    public TRPage getPrePage() {
        ...
        LogUtils.e("end" + (mBookUtil.getPosition() - 1) + "");
        trPage.setLines(getPreLines());
        LogUtils.e("begin" + mBookUtil.getPosition() + "");
    ...
    public TRPage getPageForBegin(long begin) {
        ...
        trPage.setIndex(getPageByBegin(begin));
    ...
    public TRPage getFirstPage() {
        TRPage trPage = new TRPage();
        trPage.setBegin(0);
        trPage.setIndex(0);

        mBookUtil.setPostition(0 - 1);
        trPage.setLines(getNextLines());
        trPage.setEnd(mBookUtil.getPosition());
        return trPage;
    }
    ...
    public TRPage getPageByIndex(int index) {
        mBookUtil.setPostition(0 - 1);
        int current = 0;
        TRPage trPage = null;
        while (true) {
            ...
            trPage.setIndex(current);
            List<String> lines = getNextLines();
    ...
    public int getPageByBegin(long begin) {
        mBookUtil.setPostition(0 - 1);
        int current = 0;
        while (true) {
            List<String> lines = getNextLines();
            if (mBookUtil.getPosition() > begin) {
                break;
            }
            current++;
        }
    ...
    public int getPageInfo() {
        ...
        while (true) {
            List<String> lines = getNextLines();
            if (mBookUtil.getPosition() >= mBookUtil.getBookLen()) {
                break;
            } else {
...
//            LogUtils.e(TAG + str + "   ");      // TODO: 2017/6/2 0002 wym 乱码怎么办?
        }
        return lines;
    }
    ...
    public void currentPage(Boolean updateChapter) {
        onDrawTXT(mBookPageWidget.getCurPage(), currentPage.getLines(), updateChapter);
        onDrawTXT(mBookPageWidget.getNextPage(), currentPage.getLines(), updateChapter);
    }
    ...
    public void changePage(int page) {
        currentPage = getPageByIndex(page);
        currentPage(true);
    }
